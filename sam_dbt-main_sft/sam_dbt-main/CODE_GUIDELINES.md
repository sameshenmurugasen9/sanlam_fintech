...
We adopt a structured 3-tier architecture with data lineage through bronze, silver, and gold modelling layers. This facilitates the adoption of engineering best practices such as DRY code and SOLID principles.

🥉 RAW data is staged/abstracted through Bronze models

🥈 We generate ‘canonical’ abstractions in Silver

🥇 Specific business problems are solved in Gold

## Source data
We undertake comprehensive data investigation to fully understand the data PRIOR to any modelling. This includes:
* Creating documentation to reflect our best understanding
* Identifying data quality issues (and either getting them corrected at source or via modelling)
* Specifying comprehensive tests

## Bronze models
* Aligned with data sources
* 1-1 with raw data tables
* Serve as a [facade layer](https://en.wikipedia.org/wiki/Facade_pattern) to abstract away a dependency on raw data, facilitating future changes
* Only minor transformations (e.g. type casting, renaming columns to business terminology)

## Silver models
* Apply business logic to create [canonical models](https://en.wikipedia.org/wiki/Canonical_form) based on organisation-wide standards
* For example, here we may adopt dimensional modelling (dim and fact)

## Gold models
* Transform canonical silver models to achieve a particular business purpose
* Gold models are aligned with particular use cases (e.g. BI reports/dashboards, reverse ETL)
* They are where we hand over data to the business

# Testing framework
## Source data
We specify tests on source data
* Ownership: We take ownership of data received by the dbt project prior to any modelling work.
* Documentation: We explicitly state data assumptions that are relied on by downstream models to ensure data integrity
* Data monitoring: We want to be proactively warned if data assumptions are violated so that we can either speak with upstream data owners and/or make modelling updates.

## Intermediate models
Everytime a model makes a change, relevant tests must be specified. This manages any potential modelling risks.

For example:
* CASE statements rely on a fixed set of input values (accepted_values tests should have been specified at source) that are mapped using rules. To ensure that the rules are well specified, tests should be specified on output values: e.g. not_null (to ensure no failed mappings)
* FILTERS may lead to more rows than intended being dropped. dbt_utils.at_least_one can ensure that we haven't inadvertently dropped everything.

## Gold models
The gold modelling layer is the handover point for data to the rest of the business. As such, comprehensive tests MUST be specified as part of the QA process so that we can stand behind our deliverables and build trust (which is always hard to earn and easy to lose).
...
* Tested code sets the example for everyone else.  Be a leader 💯
...
**Hard Rules** 🥇- Violating these will result in a rejected PR
Example: 
> * testability issues
> * failing builds
> * maintainability issues

**Soft Rules** 🥈- One or two should not hold up a PR, but gross violations will need to be fixed
Example: 
> * readability issues
> * formatting issues

**Unimportant** 🥉- things without any strong feelings and should never hold up a PR.  The guide may point this out simply to note that they are not of significant importance.
Example: 
...
- 🥇 Use of `SELECT DISTINCT` is not allowed.  Exceptions require architect approval.
...
- 🥇 `right join` is not allowed.  Re-write to use `left join`
